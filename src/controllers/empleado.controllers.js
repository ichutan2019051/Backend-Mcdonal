...
var Empleado = require('../models/empleado.models');
...
function setEmpleado(req, res) {
    ...
    if (userId != req.user.sub) {
        ...
        if (params.name && params.surname && params.job && params.departamento) {
            User.findById(userId, (err, userFind) => {
                if (err) {
                    return res.statis(500).send({ message: 'Error general' })
                } else if (userFind) {
                    empleado.name = params.name;
                    empleado.surname = params.surname;
                    empleado.job = params.job;
...
function getEmpleados(req, res){
    ...
    if(userId != req.user.sub){
        ...
        User.findOne({_id: userId},{'empleados': 1}).populate('empleados').exec((err, empleadosFind)=>{
            if(err){
                return res.status(500).send({message: 'Error general en el servidor'})
            }else if(empleadosFind){
                return res.send({message: 'Empleados: ', empleadosFind})               
